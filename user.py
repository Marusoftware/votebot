...
class User():
    ...
    def mkvote(self, server_id, id,users):
        ...
        pickle.dump(temp,open(self.conf_dir+str(server_id)+"_"+id,"wb"))
    def loadvote(self, server_id, id):
        return pickle.load(open(self.conf_dir+str(server_id)+"_"+id,"rb"))
    def dumpvote(self, server_id, id, obj):
        pickle.dump(obj,open(self.conf_dir+str(server_id)+"_"+id,"wb"))
    ...
    def isexist(self, server_id, id):
        return str(server_id)+"_"+id in os.listdir(self.conf_dir)
    def addmovingVote(self, server_id, id):
        dct={}
        temp=self.loadvote(server_id, id)
        if os.path.exists(self.conf_dir+str(server_id)+"_moving"):
            dct=pickle.load(open(self.conf_dir+str(server_id)+"_"+"moving","rb"))
        dct[id]=temp["name"]
        pickle.dump(dct,open(self.conf_dir+str(server_id)+"_moving","wb"))
        temp.update(status="running")
        temp.update(vote={})
        temp.update(voted={})
        self.dumpvote(server_id, id, temp)
    def getmovingVote(self, server_id):
        return list(pickle.load(open(self.conf_dir+str(server_id)+"_moving","rb")).keys())
    def getmovingVotename(self, server_id):
        return list(pickle.load(open(self.conf_dir+str(server_id)+"_moving","rb")).values())
    def getmovingVotedict(self, server_id):
        return pickle.load(open(self.conf_dir+str(server_id)+"_"+"moving","rb"))
    def closeVote(self, server_id, id):
        dct=pickle.load(open(self.conf_dir+str(server_id)+"_"+"moving","rb"))
        dct.pop(id)
        pickle.dump(dct,open(self.conf_dir+str(server_id)+"_"+"moving","wb"))
        temp=self.loadvote(server_id, id)
    ...
    def loadvote_token(self, token):
        dct=pickle.load(open(self.conf_dir+"token"+token,"rb"))
